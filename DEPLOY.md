...
4. Start the deploy (first build can take 5–10 minutes).
...
4. Add a Persistent Disk with mount path `/data` (1 GB). If disk is not available on the free plan, deploy still works; the DB resets on restart — re-run the seed script after.
...
docker run -p 8000:8000 -e GEMINI_API_KEY=... -e SECRET_KEY=dev -e DATABASE_URL=sqlite:////data/internroute.db -e CHROMA_PERSIST_DIRECTORY=/data/chroma_data -e UPLOAD_DIR=/data/uploads -v internroute-data:/data internroute
```
